      ...
      setRowData((prev) => {
        const newData = prev.data.map((product) => {
          if (product.productCode === productCode.split('+')[0]) {
            return {
              ...product,
              status: product.status === CONST.STATUS_PRODUCT.ACTIVE ? CONST.STATUS_PRODUCT.INACTIVE : CONST.STATUS_PRODUCT.ACTIVE,
            };
          }
          return product;
        });
                  ...
                  <TableBody>
                    { rowData.data && rowData.data.map((DetailRow) => (
                      <TableRow key={DetailRow.productCode}>
                        ...
                        <TableCell align="center">{DetailRow.inventory}</TableCell>
                        <TableCell align="center">
                           <FormControlLabel
                          control={
                            <Switch
                            checked={DetailRow.status === CONST.STATUS_PRODUCT.ACTIVE }
                              inputProps={{
                                "aria-label": "controlled",
                              }}
                              onChange={() => handleChangeStatusProduct(DetailRow.productCode+ '+' + rowData.code)}
                            />
                          }
                          label = {
                            DetailRow.status === CONST.STATUS_PRODUCT.ACTIVE ? "Hoạt động" : "Ngừng hoạt động"
                          }
                        />
                          </TableCell>
                          <TableCell align="center">
            ...
            >
                ...
                <Table aria-label="collapsible table">
                  <TableHead>
                    <TableRow>
                      <TableCell />
                      <TableCell className="font-bold text-lg">Mã lô</TableCell>
                      <TableCell align="center" className="font-bold text-lg">Số lượng</TableCell>
                      <TableCell align="center" className="font-bold text-lg">Mã nhà cung cấp</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {rows.map((row) => (
